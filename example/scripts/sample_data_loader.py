...
DB_FILE = '/tmp/test.db'
SQLITE_CONN_STRING = f"sqlite:///{DB_FILE}"
...
def load_table_data_from_csv(file_name):
    ...
    if conn:
        ...
        cur.executemany(
            "INSERT INTO test_table_metadata (database, cluster, "
            "schema_name, name, description) VALUES (?, ?, ?, ?, ?);",
            to_db,
        )
        conn.commit()
...
def load_col_data_from_csv(file_name):
    ...
    if conn:
        ...
        cur.execute(
            "create table if not exists test_col_metadata "
            "(name VARCHAR(64) NOT NULL , "
            "description VARCHAR(64) NOT NULL , "
...
def create_sample_job(table_name, model_name):
    ...
    task = DefaultTask(
        extractor=sql_extractor, loader=csv_loader, transformer=NoopTransformer()
    )
    ...
    job_config = ConfigFactory.from_dict(
        {
            ...
            "loader.filesystem_csv_neo4j.{}".format(
                FsNeo4jCSVLoader.SHOULD_DELETE_CREATED_DIR
            ): True,
    ...
    job_config = ConfigFactory.from_dict(
        {
            ...
            "transformer.elasticsearch.{}".format(
                ElasticsearchDocumentTransformer.ELASTICSEARCH_INDEX_CONFIG_KEY
            ): elasticsearch_new_index_key,
            "transformer.elasticsearch.{}".format(
                ElasticsearchDocumentTransformer.ELASTICSEARCH_DOC_CONFIG_KEY
...
if __name__ == "__main__":
    ...
    if create_connection(DB_FILE):
        ...
        job1 = create_sample_job(
            "test_table_metadata", "databuilder.models.table_metadata.TableMetadata"
        )
        job1.launch()

        # start col job
        job2 = create_sample_job(
            "test_col_metadata", "example.models.test_column_model.TestColumnMetadata"
        )
        job2.launch()
